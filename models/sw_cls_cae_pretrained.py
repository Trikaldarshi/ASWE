...
class sw_cls_cae_pretrained(nn.Module):

  def __init__(self, hidden_dim, embedding_dim, rnn_type, bidirectional,
                num_layers, num_classes,dropout, pre_model):
    super(sw_cls_cae_pretrained, self).__init__()

    # paratmers for attention block
    self.hidden_dim_enc = hidden_dim

    # decoder parameters
    self.hidden_dim_dec = embedding_dim # hidden_dim
    self.dec_emb_layer = embedding_dim # input dim from embedding layer

    # assert self.hidden_dim_dec==self.hidden_dim_enc
    ...
    self.attention = Attention(self.hidden_dim_enc, self.hidden_dim_dec)
    self.decoder = Decoder(self.num_classes, self.dec_emb_layer , self.hidden_dim_enc, self.hidden_dim_dec, self.dropout, self.attention)
