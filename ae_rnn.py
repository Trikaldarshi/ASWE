...
from utility_functions.awe_dataset_class import awe_dataset_pre_computed, cae_awe_dataset_pre_computed
from utility_functions.model_cae import model_cae
from utility_functions.utils_function import collate_fn, collate_fn_cae, save_checkpoints, load_checkpoints, average_precision
...
def main():
  ...
  if args.wandb:
    wandb.init(project="word2awe", dir=args.path_to_output)
  ...
  def train_model(model, train_load, val_load, n_epochs):
    ...
    for epoch in range(base_epoch, n_epochs + 1):
      ...
      for ij, (x, lens_x, _,_, y, lens_y,_,_) in enumerate(train_load): ## CAE training
        ...
        # lens_x, lens_y = lens_x.to(device), lens_y.to(device)
      ...
      # for _, (data,lens,word_name,_) in enumerate(train_load): ## AE training, uncomment this and comment the above for loop
      #   optimizer.zero_grad()
      #   data, lens = data.to(device), lens.to(device)
      #   # lens = lens.to(device)
      #   lens, perm_idx = lens.sort(0, descending=True)
      ...
      #   loss = criterion(seq_pred, data)
        ...
        # end = time.time()
        # print("Time for one batch:",ij, end-start)

      # end_epoch = time.time()
      # print("Time for one epoch:",epoch, end_epoch-start_epoch)
      val_losses = []
      model = model.eval()
      with torch.no_grad():
